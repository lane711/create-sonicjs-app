...
const getParameter = async (setting) => {
  ...
  console.log(
    `\n\nPlease enter your ${setting.label}.${
      setting.howToGenerate ? ` ${setting.howToGenerate}` : ""
    }`
  );
  const settingValue = await readSetting.question(
    `${interactiveColor}${setting.label}: ${resetColor}`
  );
  return settingValue;
};
...
const getConfigFile = async () => {
  const configFileTemplate = (
    await readFile(join(projectPath, "wrangler.example.toml"))
  ).toString();
  const configFile = configFileTemplate
    .replace("${ACCOUNT_ID}", trimAndClean(cloudflareAccountId))
    .replace("${KV_NAMESPACE_ID}", trimAndClean(cloudflareKvNamespaceId))
    .replace(
      "${KV_NAMESPACE_PREVIEW_ID}",
      trimAndClean(cloudflareKvNamespacePreviewId)
    )
    .replace("${DATABASE_ID}", trimAndClean(cloudflareDatabaseId));
  return configFile;
};

const trimAndClean = (input) => {
...
const main = async () => {
  ...
  cloudflareAccountId =
    cloudflareAccountId ?? (await getParameter(settings.ACCOUNT_ID));
  cloudflareKvNamespaceId =
    cloudflareKvNamespaceId ?? (await getParameter(settings.KV_NAMESPACE_ID));
  cloudflareKvNamespacePreviewId =
    cloudflareKvNamespacePreviewId ??
    (await getParameter(settings.KV_NAMESPACE_PREVIEW_ID));
  cloudflareDatabaseId =
    cloudflareDatabaseId ?? (await getParameter(settings.DATABASE_ID));
  ...
  try {
    ...
    console.error(
      `${errorColor}Could not read config file! aborting!${resetColor}`,
      err
    );
    process.exit(1);
  }
  try {
    ...
    console.error(
      `${errorColor}Could not write config file! This might be a permissions issue. aborting!${resetColor}`,
      err
    );
    process.exit(1);
  }
  try {
    ...
    console.warn(
      "Could not cleanup properly! Please manually delete the wrangler example file and the .scripts directory"
    );
  }
  console.log(
